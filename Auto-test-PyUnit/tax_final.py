class Caltax():
    def __init__(self, income=0, mpf=0, st_tax=0, taxs=0,
    income_H=0,income_W=0,MPF_H=0,MPF_W=0,MPF=0,tax_H=0,tax_W=0,tax=0,s_tax=0,stand_tax=0,stand_tax_H=0,stand_tax_W=0):
        ### let object for tax_s() ###
        self.income = income
        self.deductions = mpf 
        self.st_tax = st_tax # standard tax
        self.taxs = taxs # progressive tax
        ### let object for tax_j() ###
        self.income_H = income_H
        self.income_W = income_W
        self.MPF_H = MPF_H
        self.MPF_W = MPF_W
        self.MPF = MPF # H + W MPF
        self.tax_H = tax_H 
        self.tax_W = tax_W
        self.stand_tax_H = stand_tax_H
        self.stand_tax_W = stand_tax_W
        self.tax = tax # join Tax (H + W)
        self.s_tax = s_tax # total of separate tax 
        self.stand_tax = stand_tax # join tax (standard tax)
    ...
    ##############################################################
        ...
        self.MPF = MPF
    ##############################################################
        if tax_H < stand_tax_H and tax_W < stand_tax_W or tax_H == stand_tax_H or tax_W == stand_tax_W:
    ...
    ##############################################################       
            ...
            print("Deductions: ", MPF_W)
            print("The Standard Taxation: ", stand_tax_W)
            s_tax = stand_tax_H + stand_tax_W
    ##############################################################
        print("Under Separate Taxation: ",s_tax)
        if tax < stand_tax:
        ...
        if tax > s_tax:
            print("Deductions: ", MPF)
            print("Under Separate Taxation is better: ",s_tax)
        elif tax < s_tax:
            if tax < stand_tax:
                print("Deductions: ", MPF)
...
def main():
    ...
    while True:
        ...
        if choice == "1" :
            inputincome = float(input("Enter Your income: "))
            result = Caltax(income = inputincome)
            result.tax_s()
        elif choice == "2" :
            inputincome_H = float(input("Enter Your Husband Income: "))
            inputincome_W = float(input("Enter Your Wife Income: "))
            result = Caltax(income_H = inputincome_H,income_W = inputincome_W)
            result.tax_j()
        elif choice == "Q" :
            break
